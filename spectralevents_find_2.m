...
for ti=1:numTrials
    TFR_AT = squeeze(TFR(:,:,ti))./medianpower; %Above threshold TFR: first isolate 2D TFR matrix and normalize
    TFR_AT(TFR_AT<thrFOM) = 0; %Set below-threshold values to zero
    
    boundary = thresh_boundary(TFR_AT,thrFOM);
    
    [peakF,peakT] = find(imregionalmax(squeeze(TFR_AT(:,:,ti)))); %Indices of max local power
    peakpower = TFR(find(imregionalmax(squeeze(TFR_AT(:,:,ti))))+(ti-1)*flength*tlength); %Power values at local maxima (vector; compiles across frequencies and time)
...
%   if isequal(method,'method2')
%       overlapFreq_bool = Ffwhm(1:end-1,2)>=Ffwhm(2:end,1);
%       overlapTime_bool = Tfwhm(1:end-1,2)>=Tfwhm(2:end,1);
%   end